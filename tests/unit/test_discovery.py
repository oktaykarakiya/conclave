...
# --- flat-fallback seeding from the live repo file tree ---------------------


async def test_seeds_regions_from_repo_file_tree(db: Database, tmp_path: Path) -> None:
    """With no coverage rows, the sweep seeds regions from the repo's real top-level dirs.

    This pins the rework that replaced repo-knowledge layout seeding with a live ``iterdir`` of
    the working tree: top-level directories become seedable regions, ``ignore_patterns`` matches
    (``.git``, ``.venv``) and plain files are dropped, and the swept region is one of the real
    dirs — proof the file tree, not a knowledge row, drives the flat fallback.
    """
    (tmp_path / "src").mkdir()
    (tmp_path / "tests").mkdir()
    (tmp_path / ".git").mkdir()  # ignore_patterns floor — must never be seeded
    (tmp_path / ".venv").mkdir()  # ignore_patterns floor — must never be seeded
    (tmp_path / "README.md").write_text("not a dir", encoding="utf-8")  # files are not regions
    p = await repo.create_project(db, name="demo", path=str(tmp_path), default_branch="main")
    bus = EventBus(db)
    provider = _HunterProvider()
    ...
    assert found is not None
    assert found.region in {"src", "tests"}  # swept one of the real, non-ignored dirs
    seeded = {r.region for r in await repo.list_coverage_regions(db, p.id)}
    assert seeded == {"src", "tests"}  # only real dirs; .git/.venv/files excluded
...
async def test_no_region_is_noop(db: Database, tmp_path: Path) -> None:
    """An empty repo tree seeds no region, never dispatches the hunter, and discovers nothing."""
    # An empty working tree (no top-level dirs to seed) and no pre-seeded coverage rows: the
    # file-tree fallback yields nothing, so selection returns None before any dispatch.
    p = await repo.create_project(db, name="demo", path=str(tmp_path), default_branch="main")
    bus = EventBus(db)
    provider = _HunterProvider()
